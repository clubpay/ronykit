...
1. `errs.GenWrap(errs.StatusCode, "ERROR_CODE")` for wrappable sentinel errors
   that accept additional context via `err("detail text")`.
2. `errs.B().Code(errs.StatusCode).Msg("ERROR_CODE").Err()` for static sentinel
   errors.
