...
| `errors.New`, `fmt.Errorf`, third-party error libs           | `rony/errs` (`errs.GenWrap`, `errs.B().Code(...).Msg(...).Err()`)                                         | `packages/errs`, `architecture/error-handling`                       |
| `kit.Contract` / `desc.Service` in feature `api/`            | `rony.SetupOptionGroup` + `WithUnary` / `WithStream` / `WithRelay`                                        | `packages/rony`, `architecture/api-handler-files`                    |
| `net/http` server or extra gateway for feature APIs          | `rony` default fasthttp (already wired)                                                                   | `packages/rony`, `architecture/gateway-choice`                       |
| raw `kit` EdgeServer bootstrap in a scaffolded app           | `rony.NewServer` in `pkg/runner`                                                                          | `packages/kit`, `packages/rony`                                      |
...
| `sync.Map`+TTL hacks, third-party in-mem caches              | `x/cache` (Ristretto, process-local only)                                                                  | `packages/cache`, `characteristics/cache`                            |
| Redis client / shared cache                                  | `x/datasource.InitRedis` — **not** `x/cache`                                                              | `packages/datasource`, `characteristics/redis`                       |
...
| hand-written HTTP/gRPC clients to other services             | generated client stubs (`make gen-stub`) consumed via `di.StubProvider`                                   | `packages/stub`, `architecture/inter-service-stubs`                  |
...
3. The scaffolded workspace ships a `.golangci.yml` with `depguard` rules that fail `make lint` / `make verify` when forbidden imports (e.g. `go.temporal.io/sdk`, `log`, `log/slog`, `go.uber.org/zap`, `github.com/google/uuid`, `github.com/jinzhu/copier`) appear in non-test service code. Treat lint failures here as design violations, not formatting nits.
