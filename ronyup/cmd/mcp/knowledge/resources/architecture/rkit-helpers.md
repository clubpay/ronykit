...
- `rkit.ToJSON` / `rkit.FromJSON` for byte-safe serialization (and `rkit.B2S` / `rkit.S2B` for zero-copy `[]byte`/`string` conversions).
...
- `rkit.L` / `rkit.StrLines` for structured multi-line string building (used by the MCP tool result helpers).
