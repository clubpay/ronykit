...
- `api/api_account.go`, `api/api_jwt.go`, `api/api_password.go`, etc. for handler implementations.

Each `api_*.go` file defines its input/output DTOs at the top (with `json` and `swag` tags), followed by handler methods.
...
Use helper functions (`toAccount`, `toWalletAccount`) for domain-to-API type conversion within the same file.
