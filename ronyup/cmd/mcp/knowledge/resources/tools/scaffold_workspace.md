...
For `fullstack`, Go module paths are prefixed with `backend/` (e.g. `<repoModule>/backend/cmd/all-in-one`). Run `scaffold_feature` (and `go`/`make` commands) against the `backend/` directory; the design gate still finds `docs/design` at the repository root.
