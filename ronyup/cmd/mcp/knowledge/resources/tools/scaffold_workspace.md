...
- **`backend`** (default) — a Go-only workspace at `path`, containing `go.work`, `bundles.yaml`, `internal/runner/`, `cmd/service/`, `pkg/i18n/`, an empty `feature/` tree, `devops/` (optional `devops/devbox/` Kubernetes cluster), `docs/`, and a `.ai/mcp/mcp.json` for IDE integration.
- **`fullstack`** — a `backend/` + `frontend/` split. The Go workspace (`go.work`, `bundles.yaml`, `internal/runner/`, `cmd/service/`, `pkg/`, `feature/`, `Makefile`, `.golangci.yml`) is created under `backend/`, while `devops/`, `docs/`, and the AI assistant config (`.ai/`, `.agents/`, `.cursor/`, `AGENTS.md`) stay at the root and are shared. A framework-agnostic `frontend/` placeholder is created for the web/mobile app.
