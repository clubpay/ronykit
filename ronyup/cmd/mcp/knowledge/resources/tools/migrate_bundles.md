...
Upgrade an **existing** workspace created before executable bundles to the current layout (`internal/runner`, `bundles.yaml`, thin `cmd/service/main.go`).

## When to use

- The workspace was scaffolded with an older `ronyup` that inlined bootstrap code in `cmd/service/main.go`.
- The repo is missing `internal/runner/`, `bundles.yaml`, or still has `cmd/service/middleware.go` / `healthz.go`.
...
- **Does not touch** feature business code under `feature/*` or `pkg/*` (except `go work use` for `internal/runner`).
...
1. Copy `internal/runner/` from the embedded scaffold (shared bootstrap).
2. Rewrite `cmd/service/main.go` to delegate to `internal/runner`.
3. Remove legacy `cmd/service/middleware.go` and `healthz.go`.
4. Create `bundles.yaml` when missing (default `service` bundle with `"*"`).
5. Initialize `internal/runner` module, `go work use ./internal/runner`, and regenerate bundle import lists.
