...
func acquireConnInfo(reqCtx *fasthttp.RequestCtx) *connInfo {
	...
	reqCtx.Request.Header.VisitAll(
		func(key, value []byte) {
			...
			default:
				if !ignoredHeaders[utils.B2S(key)] {
	...
	mt.SetClientIP(utils.S2B(reqCtx.RemoteIP().To4().String()), true)

	if reqCtx.Request.Header.ConnectionUpgrade() {
		mt.upgrade = true
	} else if bytes.Equal(reqCtx.Request.Header.Peek("X-Rony-Upgrade"), ronyUpgrade) {
		reqCtx.Request.Header.SetBytesKV(utils.S2B(fasthttp.HeaderConnection), ronyUpgrade)
		mt.upgrade = true
	}
