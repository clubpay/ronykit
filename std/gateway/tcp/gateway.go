...
func (g *Gateway) Shutdown() {
	...
	for id, c := range g.conns {
		g.cfg.Logger.Info("Conn Stalled",
			zap.Uint64("ID", id),
			zap.Duration("SinceStart", time.Duration(utils.CPUTicks()-atomic.LoadInt64(&c.startTime))),
			zap.Duration("SinceLastActivity", time.Duration(utils.CPUTicks()-(atomic.LoadInt64(&c.lastActivity)))),
		)
	}
	g.connsMtx.Unlock()
}
...
func (g *Gateway) websocketHandler(c net.Conn, meta *connInfo) {
	...
	if _, err := g.upgradeHandler.Upgrade(c); err != nil {
		if ce := g.cfg.Logger.Check(log.InfoLevel, "got error in websocket upgrade"); ce != nil {
			ce.Write(
				zap.String("IP", utils.B2S(meta.clientIP)),
				zap.String("ClientType", utils.B2S(meta.clientType)),
				zap.Error(err),
			)
		}
		_ = c.Close()
